...
describe("GET /api/admin/audit/export", () => {
  ...
  beforeEach(() => {
    ...
    // The route itself imports getServerSession from "next-auth" directly
    const { getServerSession: getServerSessionDirect } = jest.requireMock("next-auth");
    (getServerSessionDirect as jest.Mock).mockResolvedValue(mockAdminSession);
  ...
  it("requires authentication", async () => {
    ...
    const { getServerSession: getServerSessionDirect } = jest.requireMock("next-auth");
    (getServerSessionDirect as jest.Mock).mockResolvedValue(null);
