 ...
 *
 * Tests: app/api/admin/dashboard-stats/route.ts
...
// Controllable mock for requireAdminAppRouter
...
describe("GP Dashboard Stats API (pages/api/admin/dashboard-stats.ts)", () => {
  ...
  beforeEach(() => {
    ...
    // Default: requireAdminAppRouter succeeds with admin context
    ...
    // Default userTeam for cross-fund scoping
  ...
  describe("Authentication", () => {
    ...
    it("should reject non-GP users with 403", async () => {
      mockRequireAdminAppRouter.mockResolvedValue(
        NextResponse.json({ error: "GP access required" }, { status: 403 })
      );
  ...
  describe("Error handling", () => {
    it("should return 500 with generic message on error", async () => {
      (mockPrisma.userTeam.findMany as jest.Mock).mockRejectedValue(new Error("DB error"));
