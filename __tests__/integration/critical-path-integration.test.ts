...
describe("Section 4: Rate Limiting Enforcement", () => {
  ...
  it("wire proof enforces upload rate limit", async () => {
    const { appRouterUploadRateLimit } = jest.requireMock("@/lib/security/rate-limiter");
    // Simulate rate limit exhaustion — App Router rate limiters return a NextResponse when blocked
    const { NextResponse } = require("next/server");
    appRouterUploadRateLimit.mockResolvedValueOnce(
    ...
    // App Router wire-proof uses appRouterUploadRateLimit (returns null or NextResponse)
    expect(appRouterUploadRateLimit).toHaveBeenCalled();
    expect(res._getStatusCode()).toBe(429);
  });
});
...
describe("Section 9: Fund Dashboard Aggregation", () => {
  ...
  it("correctly aggregates platform + manual investments", async () => {
    ...
    // The route uses prisma.fund.findMany with include: { investments, distributions, capitalCalls }
    // and then reduces over fund.investments / fund.distributions arrays directly.
    mockPrisma.fund.findMany.mockResolvedValue([
      {
        ...
        closingDate: null,
        investments: [
          { investorId: "inv-1", commitmentAmount: 200000, fundedAmount: 100000 },
          { investorId: "inv-2", commitmentAmount: 150000, fundedAmount: 100000 },
          { investorId: "inv-3", commitmentAmount: 150000, fundedAmount: 100000 },
        ],
        distributions: [
          { totalAmount: 50000 },
        ],
    ...
    // Manual investments: $200K committed, $150K funded, 2 investors
    mockPrisma.manualInvestment.findMany.mockResolvedValue([
      { fundId: FUND.id, investorId: "inv-4", commitmentAmount: 100000, fundedAmount: 75000 },
      { fundId: FUND.id, investorId: "inv-5", commitmentAmount: 100000, fundedAmount: 75000 },
    ]);
    ...
    // Investor count from combined platform (3) + manual (2) unique investors
    expect(data.funds[0].investorCount).toBe(5);
