...
jest.mock("@/lib/auth/rbac", () => ({
  ...
  enforceRBACAppRouter: jest.fn().mockResolvedValue({
    ...
    teamId: "team_1",
    role: "ADMIN",
    session: { user: { id: "gp_user_1", email: "gp@example.com" } },
  }),
  ...
  requireLPAuthAppRouter: jest.fn().mockResolvedValue({
    ...
    session: { user: { id: "lp_user_1", email: "lp@example.com" } },
  }),
}));
...
describe("GP Document Review Flow Wiring (P0-6)", () => {
  ...
  describe("LP Document Upload (POST /api/lp/documents/upload)", () => {
    ...
    it("should create document with UPLOADED_PENDING_REVIEW status", async () => {
      ...
      // User with investor profile (needed by getLPDocumentAuthContextAppRouter)
      mockPrisma.user.findUnique.mockResolvedValue({
        id: "lp_user_1",
        email: "lp@example.com",
        role: "USER",
        investorProfile: { id: TEST_INVESTOR_ID, fundId: TEST_FUND_ID },
        teams: [],
      });
